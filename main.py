...
async def home(request: Request):
    return templates.TemplateResponse(
        "home.html", {"request": request, "server_root": SERVER_ROOT})
    ...
    while True:
        ...
        try:
            ...
            if singal_text.startswith("gothca"):
                ...
                continue
            elif singal_text.startswith("transcript_upload:"):
                ...
                msg = f'TRA_{singal_text}的文字提取完成\n{transcription["text_plus_timeline"]}'
            elif singal_text.startswith("transcript_local:"):
                save_path = singal_text.split(':')[1]

                logger.debug(f"【客户端{token}】本地文件名：{save_path}")
                # 提取语音文字
                transcription = transcript_with_segments(save_path)
                ...
                # callback_code = str(uuid.uuid4())
                msg = f'SEP_{singal_text}的音频分离完成\n{new_audio[0]}\n{new_audio[1]}'
